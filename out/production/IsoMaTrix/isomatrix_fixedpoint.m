        ...
        for d = [1,2]
            ...
            if (i < j)
                ...
                if (Ap(2,2) >= Ap(1,2))
                    ...
                    plot(x_point + delta(1),y_point + delta(2),'o','LineWidth', 1,'MarkerSize', us_ms,'MarkerFaceColor',[1,1,1],'MarkerEdgeColor',color);
    %                 plot(x_point + delta(1),y_point + delta(2),'o','LineWidth', 1,'MarkerSize', us_ms,'Color',color);hold on; % (left side is stable)
                end



                %% plot over all three axes, just for cleanliness:
    ...
    if ((p_star > 0) && (q_star > 0))
        if ((p_star < 1) && (q_star < 1))
            if (((p_star+q_star) < 1))
                ...
                [x_point,y_point] = UVW_to_XY(x);
                plot(x_point,y_point,'s','MarkerSize', s_ms/4,'MarkerFaceColor',color,'MarkerEdgeColor',color);hold on; % (left side is stable)
