...
import HAL.GridsAndAgents.AgentSQ3D;

class Cell3D extends AgentSQ3D<HALMatrixGame3D> {
    ...
    void UpdateCellFitness() {
        int n = G.MapOccupiedHood(G.neighborhood,Xsq(),Ysq(),Zsq());
        fitness = 0.0;
        for (int i = 0; i < n; i++) {
    ...
    public void ReplaceProportionalToFitness() {
        ...
        for (int j = 0; j < nNeighbors; j++) {
            Cell3D neighbor_cell = G.GetAgent(G.neighborhood[j]);
